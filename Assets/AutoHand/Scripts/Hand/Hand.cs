            ...
            if (holdingObj != null)
            {
                ...
                if (grabTime > 0 && !holdingObj.instantGrab)
                {
                    ...
                    if (grabPose == null)
                    {
                        ...
                        for (float i = 0; i < grabTime * startGrabDist; i += Time.unscaledDeltaTime)
                        {
                            if (holdingObj != null)
                            {
        ...
        protected virtual void CheckHighlight()
        {
            if (holdingObj == null)
            {
                ...
                if (dir != Vector3.zero)
                {
                    ...
                    rotationOffset = Quaternion.RotateTowards(rotationOffset, Quaternion.FromToRotation(palmTransform.forward, hit.point - transform.position), 50f * Time.unscaledDeltaTime * lookAssistSpeed);
                }
        ...
        protected virtual void DeterminPose()
        {
            ...
            if (!holdingObj && !disableIK && !grabPose && handPoseArea == null && handAnimateRoutine == null)
            {
                ...
                if (currGrip != grip)
                {
                    ...
                    currGrip += ((currGrip < grip) ? Time.unscaledDeltaTime : -Time.unscaledDeltaTime) * (Mathf.Abs(currGrip - grip) * 25);
